...
// couche d'accès sqlite pour le panier - n'est jamais utilisée directement depuis les activités,
// toujours via PanierManager
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // à incrémenter si le schéma de la table change - déclenche onUpgrade()
    ...
    // stratégie de migration simple : drop + recreate
    // acceptable ici car le panier est une donnée temporaire - perdre le contenu n'est pas bloquant
    @Override
    ...
    // retourne false si le film est déjà dans le panier -
    // la vérification est manuelle car sqlite lèverait une exception sans message clair sur la clé primaire
    public boolean ajouterFilm(Film film) {
        ...
        // les listes sont sérialisées en csv - les colonnes existent pour conserver une trace
        // mais ne sont pas relues (PanierActivity n'a besoin que des infos de base)
        ...
        // rental_id stocké pour pouvoir appeler DELETE /cart/{rentalId} lors de la suppression
    ...
    // les listes (réalisateurs, acteurs, catégories) ne sont pas rechargées -
    // PanierActivity affiche uniquement le titre, l'année, la durée et la classification
    public List<Film> obtenirFilms() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                // rental_id rechargé pour permettre DELETE /cart/{rentalId}
